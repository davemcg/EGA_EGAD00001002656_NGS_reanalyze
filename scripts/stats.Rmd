...
set.seed(115470)
all_NOT_PATH_CUT <- all_NOT_PATH# %>% sample_n(nrow(all_PATH_10X) * 1000)
...
set.seed(825)
rfFit_tissue <- train(x=train_set %>% select(-variant_id, -Status, -Complicated_Status) %>% data.frame(), 
                      y = train_set$Status, 
                      method = "rf", 
                      trControl = fitControl)

selectedIndices <- rfFit_tissue$pred$mtry == rfFit_tissue$bestTune[1,] #26

g <- ggplot(rfFit_tissue$pred[selectedIndices, ], aes(m=M, d=factor(obs, levels = c("R", "M")))) + 
  geom_roc(n.cuts=0) + 
  coord_equal() +
  style_roc()
