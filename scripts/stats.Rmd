...
>>>>>>> 8cfe3c1a4fc2c7c9f238ddd7b0fff23677a9755c
```

# Plot status breakdown by disease
```{r}
metadata %>% mutate(Type = Complicated_Status) %>% 
  filter(Complicated_Status != 'Not Pathogenic') %>%  
  ggplot(aes(x=Phenotype, fill=Type)) + geom_bar(position = 'dodge') + 
